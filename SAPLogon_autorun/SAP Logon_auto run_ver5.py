...
def find_and_click(image_path, timeout=30, check_interval=0.5):
    """Kép megtalálása és rákattintás, várakozás az elem megjelenésére."""
    ...
    print(f'{image_path} - GUI elem nem található a megadott időn belül!')
    return False
...
# Fő függvény
def run(ertek, outFile, wait_time=0.1, open=False):
...
def main():
    ...
    # Felugró ablak a várakozási idő megadásához
    root = Tk()
    root.withdraw()  # Az alapablak elrejtése
    wait_time_input = simpledialog.askfloat("Várakozási idő", "Adj meg egy várakozási időt másodpercben (pl. 0.2):", minvalue=0.0, maxvalue=10.0, initialvalue=0.8)
    if wait_time_input is None:  # Ha a felhasználó nem ad meg értéket, akkor alapértelmezett érték
        wait_time_input = 0.1
    ...
    with open(file_path, "r") as file, open("output.txt", "a+") as output:
        ...
        for line in lines:
            error = run(line.strip(), output, wait_time=wait_time_input, open=error)
            time.sleep(0.1)
